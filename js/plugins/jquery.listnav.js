...
(function($) {
	$.fn.listnav = function(options) {
		...
		opts.prefixes = $.map(opts.prefixes, function(n) { return n.toLowerCase(); }); return this.each(function() {
			...
			function addLetterClass(firstChar, $el, isPrefix) { 
				...
				if (counts[firstChar] == undefined) counts[firstChar] = 0; counts[firstChar]++; 
			...
			function bindHandlers() {
				...
				$('a', $letters).click(function() {
					$('a.ln-selected', $letters).removeClass('ln-selected'); var letter = $(this).attr('class').split(' ')[0]; if (letter == 'all') { $list.show(); isAll = true; } else {
						if (isAll) { $list.hide(); isAll = false; } else if (prevLetter != '') $('#listin tr.ln-' + prevLetter).hide(); var count = getLetterCount(this); if (count > 0) { $('#listin tr.ln-' + letter).show(); }						  
						prevLetter = letter;
					}
			...
			function createLettersHtml() {
				var html = []; 
				for (var i = 1; i < letters.length; i++) {if (html.length == 0) html.push('<a class="all" href="#">ALL</a><a class="_" href="#">0-9</a>'); html.push('<a class="' + letters[i] + '" href="#">' + ((letters[i] == '-') ? '...' : letters[i].toUpperCase()) + '</a>'); }
	...
	}; $.fn.listnav.defaults = { initLetter: '', includeAll: true, incudeOther: false, includeNums: true, flagDisabled: true, noMatchText: '', showCounts: false, cookieName: null, onClick: null, prefixes: [] };
})(jQuery);
