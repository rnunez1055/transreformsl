...
class Tzn {
	...
	function setAuto(&$data, $nested = '', $method = '_setFromHttp') {
		...
		foreach ( $this->_properties as $key => $type ) {
			if ($type == 'LVL' && ($method == '_setFromHttp')) {
				...
				// echo "$nested: setting $key as ".$this->$key." ($nKey)
			// <br/>";
				// }
	...
	function setNum($key, $value, $negative = 0) {
		$value = preg_replace ( array ('/[a-zA-Z]/', '/ /', '/,/' ), '', $value );
		if (! $negative) {
	...
	function qCheckbox($keyval, $default = 0, $style = '', $xtra = '') {
		$value = Tzn::getBol ( $keyval, $default );
		$form = '<input type="checkbox" id="c_' . $keyval . '" name="' . $keyval . '" ';
		$form .= Tzn::_style ( $style );
		if ($value) {
			$form .= 'checked="true" ';
		}
	...
	function qSelect2($name, $keyval, $default = '', $nochoice = '', $style = 'tznFormSelect', $xtra = '') {
		$form = '<select name="' . $name . '" ';
		$form .= Tzn::_style ( $style );
		if ($xtra) {
			$form .= $xtra . ' ';
	...
	function qDateTime($keyval, $tz = TZN_TZDEFAULT, $default = '', $style = 'tznFormDate', $xtra = '') {
		...
		$form = ', <input type="text" name="' . $name . 'Time" value="' . substr ( $value, 11, 5 ) . '" ';
		if (is_int ( $style )) {
			$form .= 'size="' . $style . '" ';
		} else {
			$form .= Tzn::_style ( $style );
